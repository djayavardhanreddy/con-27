...
export default function AgendaTimeline({ agenda }: AgendaTimelineProps) {
  ...
  const showTimeline = false; // Set to true to show the interactive agenda timeline

  return (
    <Stack gap="xl" align="center" style={{ width: '100%' }}>
      {!showTimeline ? (
        <Card p="xl" style={{ width: '100%', maxWidth: '600px', textAlign: 'center', background: 'var(--glass-bg)', border: '1px solid var(--glass-border)', borderRadius: '16px', boxShadow: 'var(--glass-shadow)' }}>
          <Text fw={600} size="md">
            📅 The detailed program agenda is being finalized and will be updated shortly.
          </Text>
        </Card>
      ) : (
        <>
